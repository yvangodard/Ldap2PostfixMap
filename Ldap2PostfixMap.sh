...
alldone () {
	...
	[ $EMAIL_LEVEL -ne 0 ] && [ $1 -ne 0 ] && cat $LOG_TEMP | mail -s "[ERROR : ldap2postfixmap.sh] list $LISTNAME (LDAP group $LDAPGROUP,$DNBASE)" ${EMAIL_ADDRESS}
	[ $EMAIL_LEVEL -eq 2 ] && [ $1 -eq 0 ] && cat $LOG_TEMP | mail -s "[OK : ldap2postfixmap.sh] list $LISTNAME (LDAP group $LDAPGROUP,$DNBASE)" ${EMAIL_ADDRESS}
...
if [[ -z $(cat ${VIRTUAL_MAP_FILE_NEW}) ]]
	then
	echo -e "\t-> Nothing to import in ${VIRTUAL_MAP_FILE}"
else
	mv ${VIRTUAL_MAP_FILE} ${VIRTUAL_MAP_FILE}.old
	mv ${VIRTUAL_MAP_FILE_NEW} ${VIRTUAL_MAP_FILE}
